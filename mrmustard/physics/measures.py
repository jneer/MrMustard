...
def fidelity(a: State, b: State) -> float:
    ...
    if a.is_gaussian and b.is_gaussian:
        return core.gaussian.fidelity(a.means, a.cov, b.means, b.cov, hbar=settings.HBAR)
    else:
        # fock representation is automatically generated
        return core.fock.fidelity(a.fock, b.fock, a_pure=a.is_pure, b_pure=b.is_pure)
...
def number_var(self) -> Matrix:
    r"""
    Returns the diagonal of the photon number covariance matrix.
    """
    if a.is_gaussian:
        return gaussian.number_var(a.cov, a.means, settings.HBAR)
    else:
        raise NotImplementedError("number_var not yet implemented for non-Gaussian states")